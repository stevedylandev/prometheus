...
import { ethAsset, defaultAssets } from "@/lib/constants";
...
export function useTokenList({
	...
	const result = useQuery({
		...
		queryFn: async () => {
			const url = `https://base-sepolia.blockscout.com/api/v2/addresses/${userAddress}/tokens?type=ERC-20`;
			...
			const defaultTokens = defaultAssets[chainId] ?? [];

			// Transform the data to match our AssetWithBalance type (without balance)
			const tokens = data.items.map((item) => {
				const tokenAddress = item.token.address as Address.Address;

				// Check if this token exists in our default assets
				const defaultToken = defaultTokens.find(
					(token) => token.address.toLowerCase() === tokenAddress.toLowerCase(),
				);

				// If it exists in defaults, use that data (with better icons, prices, etc.)
				if (defaultToken) {
					return defaultToken;
				}
